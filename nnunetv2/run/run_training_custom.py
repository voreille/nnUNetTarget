...
from nnunetv2.training.nnUNetTrainer.variants.finetuning.nnUNetTrainerFineTuning import (
    nnUNetTrainerWithConfig,
)
...
def get_trainer_from_args(
    ...
    if issubclass(nnunet_trainer, nnUNetTrainerWithConfig):
        if trainer_config is None:
            print(
                "\n############################\n"
                "INFO: you use a custom trainer and no config was provided using default one..."
                "\n############################\n"
            )
            trainer_config = "nnunetv2/training/nnUNetTrainer/variants/finetuning/configs/default_finetuning_v1.yaml"
        print(
                f"\n############################\n"
                f"INFO: you use a custom trainer : {type(nnunet_trainer.__class__.__name__)} "
                f"\n############################\n"
            )
        ...
        nnunet_trainer.trainer_config = trainer_config
    else:
        print(
            "\n############################\n"
            "INFO: you use a default trainer with a config so it won't use the config."
            "\n############################\n"
        )
        nnunet_trainer = nnunet_trainer(
            plans=plans,
...
def run_training_entry():
    ...
    # Added to pass config to custom nnUneTrainer
    parser.add_argument(
        "-trainer_config",
        type=str,
        default=None,
        required=False,
        help="[OPTIONAL] path to nnUNetTrainer config file to be used as with custom trainer.",
    )
